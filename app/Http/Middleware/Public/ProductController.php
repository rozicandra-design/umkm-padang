...
class ProductController extends Controller
{
    ...
    {
        $product  = Product::where('slug', $slug)->with('umkm.category','images','reviews.customer','category')->firstOrFail();
        $product->increment('view_count');
        $related  = Product::available()->where('id','!=',$product->id)->where('category_id',$product->category_id)->take(4)->get();
